...
- Query 1: 30 ms → 0.447 ms → ускорение ~67x  
- Query 2: 17 ms → 25 ms → ухудшение (из-за большого диапазона)  
- Query 3: 1728 ms → 1686 ms → без улучшения  
- Query 4: 817936 ms → 556 ms → ускорение ~1470x  
...
- создана таблица `orders_partitioned`
- созданы партиции по кварталам (2024–2025)
- выполнен перенос данных (100 000 строк)
- созданы индексы
- выполнен ANALYZE
...
- используется partition pruning
- запрос обращается только к нужной партиции
- время: ~3.9 ms
...
- Query 1: 30 ms → 0.447 ms → 0.195 ms  
- Query 2: 20 ms → 25 ms → 21 ms  
- Query 3: 1728 ms → 1686 ms → 1233 ms  
- Query 4: 817936 ms → 556 ms → 489 ms  
...
Q3 (JOIN + GROUP BY):
- индекс не устраняет полную агрегацию
- сортировка остаётся узким местом
- используется external merge (диск)

Причины:
- большой объём данных
- тяжёлый GROUP BY
- требуется pre-aggregation (materialized view)
