...
-- Индекс 4
-- ============================================
-- Ускоряет: Q2 — фильтрация по широкому диапазону дат без фильтра по status.
-- BRIN хранит min/max значения по блокам страниц (8 kB каждый).
-- Почему BRIN, а не BTREE:
--   - Данные вставлялись примерно в хронологическом порядке (seed генерирует
--     случайные даты, но в реальных системах created_at коррелирует с физическим
--     порядком строк) → BRIN эффективен.
--   - BRIN занимает на 2–3 порядка меньше места, чем BTREE на 100k строках.
--   - Подходит для отчётных запросов с большими диапазонами дат.
-- Примечание: при случайном порядке вставки (как в seed) BRIN менее эффективен;
--             оставлен как учебный пример альтернативного типа индекса.
